...
export default function BookingConfirmationPage() {
  ...
  const billableDaysParam = searchParams.get('billableDays');
  const estimatedTotalParam = searchParams.get('estimatedTotal');

  const pricePerDay = pricePerDayParam ? Number(pricePerDayParam) : null;
  const fallbackBillableDays = getBillableDays(pickupAt, returnAt);
  const billableDays = billableDaysParam ? Number(billableDaysParam) : fallbackBillableDays;
  const estimatedTotal = estimatedTotalParam
    ? Number(estimatedTotalParam)
            ...
            <div className="mt-6 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 space-y-3 text-sm text-neutral-700">
                ...
                <p className="text-xs text-neutral-500">
                  Pricing shown as an estimate using the locked-in booking snapshot captured at reservation time.
                </p>
              </div>
            </div>
